...
variable "vpc_id" {
  description = "VPC ID where ECS cluster and resources will be deployed"
  type        = string
}

variable "subnets" {
  description = "Subnet IDs for ECS tasks / EC2 instances"
  type        = list(string)
}
...
EOT
  ...
  type = list(object({
    ...
    # Task-level settings
    cpu    = number # task-level CPU units
    memory = number # task-level memory MiB
...
variable "execution_role_custom_policies" {
  description = "Custom policies to attach to the ECS **task execution role**. Supports both ARN (managed policies) and inline JSON policies."
  type        = map(any) # key = policy name, value = ARN string or JSON policy document string
  default     = {}
}
...
variable "alb_security_group_ingress_rules" {
  description = "Ingress rules for ALB security group (e.g., HTTP/HTTPS from 0.0.0.0/0)"
  type = list(object({
    from_port   = number
    to_port     = number
    protocol    = string
...
variable "alb_listener_port" {
  description = "ALB listener port (80 for HTTP, 443 for HTTPS)"
  type        = number
  default     = 80
}
